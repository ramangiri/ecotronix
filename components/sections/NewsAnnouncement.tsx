          ...
          <Carousel
            ...
            items={newsItems.map((news, index) => (
              <a
                ...
                className="bg-white rounded-xl p-8 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between min-h-[240px] h-full group block"
              >
                <h3 className="text-gray-800 text-lg md:text-xl font-medium leading-relaxed line-clamp-3 mb-8">
                  {news.title}
                </h3>
                <div className="flex items-center justify-between mt-auto">
                  <span className="bg-[#E5E5E5] text-gray-600 text-xs font-medium px-3 py-1.5 rounded-sm">
                    {news.date}
                  </span>
                  <div className="flex items-center gap-1 text-[#0066ff] font-medium text-sm group-hover:underline">
                    더보기
