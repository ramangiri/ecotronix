        ...
        <div className="mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
            ...
            <div className="w-full md:w-[400px] flex items-center">
              <input
                ...
                className="w-full h-12 px-4 bg-[#F8F8F8] border border-gray-200 rounded-l-md focus:outline-none focus:ring-1 focus:ring-[#0066ff] text-gray-700"
              />
              <button className="h-12 w-14 bg-[#0066ff] text-white rounded-r-md flex items-center justify-center hover:bg-blue-700 transition-colors shrink-0">
          ...
          <div className="flex flex-col gap-4 mb-16 min-h-[400px]">
            {filteredResources.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-500 bg-[#F8F8F8] rounded-lg border border-gray-200/60 h-full">
              ...
              currentItems.map((item) => (
                <div key={item.id} className="bg-[#F8F8F8] border border-gray-200/60 rounded-lg p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:shadow-sm transition-shadow">
                  <div>
                    <h3 className="text-lg font-medium text-gray-900 mb-2">{item.title}</h3>
                    <p className="text-sm text-gray-400">{item.date}</p>
                  </div>
                  <div className="flex items-center gap-2 w-full md:w-auto">
                    <button className="flex-1 md:flex-none px-6 py-3 bg-white border border-gray-200 rounded text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                      자료 다운로드
                    </button>
                    <button className="w-12 h-12 bg-[#1A1A1A] text-white rounded flex items-center justify-center hover:bg-black transition-colors shrink-0">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center rounded transition-colors font-medium ${currentPage === page
